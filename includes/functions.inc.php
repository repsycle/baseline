<?PHP
    ...
    function array2options($array, $default=null)
    {
        ...
        {   
            if(!is_null($default) && $row == $default)
                $out .= tab() . tab() . '<option value="' . htmlspecialchars($row, ENT_QUOTES) . '" selected="selected">' . ucfirst($row) . '</option>'.nl();
            elseif(is_array($default) && in_array($row,$default))
                $out .= tab() . tab() . '<option value="' . htmlspecialchars($row, ENT_QUOTES) . '" selected="selected">' . ucfirst($row) . '</option>'.nl();
            else
                $out .= tab() . tab() . '<option value="' . htmlspecialchars($row, ENT_QUOTES) . '">' . ucfirst($row) . '</option>'.nl();
        }
        return $out;
    }
    ...
    function mdy($date = null, $prefix = null, $output_format = 'm d y')
    {
        ...
        $month_dd = "<select class='span2' name='{$prefix}month' id='{$prefix}month'>" . nl();
        for($i = 1; $i <= 12; $i++)
        {
        ...
        $day_dd = "<select class='span1' name='{$prefix}day' id='{$prefix}day'>" . nl();
        for($i = 1; $i <= 31; $i++)
        {
        ...
        $year_dd = "<select class='span1' name='{$prefix}year' id='{$prefix}year'>" . nl();
    ...
    function send_html_mail($to, $subject, $message, $from, $plaintext = 'To view the message, please use an HTML compatible email viewer!')
