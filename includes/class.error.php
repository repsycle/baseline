<?PHP
    class Error
    {
        ...
        {
            if(isset($_SESSION['error'][$id]) && !is_array($_SESSION['error'][$id]))
                $_SESSION['error'][$id] = array($msg);
            else
        ...
        public function css($header = true)
        {
            ...
            if(count($_SESSION['error']) > 0)
            {
                ...
                $out .= "#" . implode(", #", array_keys($_SESSION['error'])) . " { {$this->style} }";
                if($header) $out .= '</style>';
            }
            echo $out;
        }
        ...
        {
            if(count($_SESSION['error']) == 0) return '';
            ...
            if(count($_SESSION['error']))
            {
                ...
                foreach($_SESSION['error'] as $type=>$error)
                {
                    ...
                    $out .= "<div class='alert alert-".$type."'>
                            <a class='close' data-dismiss='alert' href='#'>×</a>" . $addError . implode('<br />', $error) . "</div>";
                }
                unset($_SESSION['error']);
                return $out;
            }
        ...
        public function email($val, $inSystem=true)
        {            
            // Preg match as well as do a MX lookup
            if(preg_match("/^([_a-z0-9+-]+)(\.[_a-z0-9-]+)*@([a-z0-9-]+)(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/i", $val))
            {
                if (($inSystem == true)
                && (Auth::emailExists($val)))
                {
                    $this->add('error', 'The email address already in use.');
                    return false;  
                }
            ...
            // No preg match found            
                ...
                $this->add('error', 'The email address you entered is not valid.');
                return false;
            }        
        }
        ...
        public function captcha($value)
        {
           ...
           && (!empty($_SESSION['security_number']))
           && ($_SESSION['security_number']==$value))
           {
              return true;
           } else {
              $this->add('error', 'Invalid captcha provided!');
              return false;         
           }
