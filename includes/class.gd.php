<?PHP
    ...
    class GD
    {
        ...
        public function wallpaper($image)
        {
            $sizes = array();	
            $sizes['width'] = array('2560', '1920', '1680', '1600', '1440', '1280', '1024', '800');
            $sizes['height'] = array('1600', '1200', '1050', '1200', '960', '1024', '768', '600');
                        
            foreach($sizes['width'] as $key=>$width)
            {
                $height = $sizes['height'][$key];
                $this->resizeToResolution($image, $width, $height);
            }
        }
                
        public function availableWallpapers($image) // returns an array of possible resolutions
        {
            $sizes = array();	
            $sizes['width'] = array('2560', '1920', '1680', '1600', '1440', '1280', '1024', '800');
            $sizes['height'] = array('1600', '1200', '1050', '1200', '960', '1024', '768', '600');
        ...
        public function resizeToResolution($image, $width, $height, $quality=90, $return=true)
        {
            $this->loadFile($image);
            $imgWidth = $this->width;
            ...
            {            
                ...
                $image_aspect = round($imgWidth / $imgHeight, 2);
                
                $new_name =  str_replace('.jpg', "_" . $width . "_" . $height . ".jpg", $image);
                ...
                if ($return)
                {
                    ...
                    $this->saveAs($new_name, 'jpg', $quality); 
                }
